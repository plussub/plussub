var srtPlayer = srtPlayer || {};
if (typeof exports !== 'undefined') {
    exports.srtPlayer = srtPlayer;
    var messageBus = null;
    srtPlayer.Descriptor = require('./../../Descriptor.js').srtPlayer.Descriptor;
}

srtPlayer.MovieInformationService = srtPlayer.MovieInformationService || ((messageBusLocal = messageBus, fetch = window.fetch) => {

        const SERVICE_CHANNEL = messageBusLocal.channel(srtPlayer.Descriptor.CHANNEL.SERVICE);
        const SERVICE_CONST = srtPlayer.Descriptor.SERVICE.MOVIE_INFORMATION;

        SERVICE_CHANNEL.subscribe({
            topic: SERVICE_CONST.SUB.SEARCH,
            callback: loadData
        });

        async function loadData(query) {


            try {
                const response = await fetch('https://0e53p7322m.execute-api.eu-central-1.amazonaws.com/release/movie/search/' + decodeURIComponent(query));
                if (response.status !== 200) {
                    console.log('Invalid Status Code: ' + response.status);
                    return;
                }
                const results = await createOmdbInformationFrom(createImbdInformationFrom(await response.json()));

                SERVICE_CHANNEL.publish({
                    topic: SERVICE_CONST.PUB.SEARCH_RESULT,
                    data: results
                })

            } catch (err) {
                SERVICE_CHANNEL.publish({
                    topic: srtPlayer.Descriptor.SERVICE.NOTIFICATION.SUB.NOTIFY,
                    data: {
                        msg: "Something goes wrong with the video search"
                    }
                });
            }
        }

        function createImbdInformationFrom(imdbResponse) {
            return Object.keys(imdbResponse).map((k) => imdbResponse[k]).reduce((p, c) => {
                p.push(...c);
                return p;
            }, []);
        }

        async function createOmdbInformationFrom(imdbResult) {

            const missingPosterFb = "data:image/png;base64,";
            const maxRequestedImdb = 10;
            const imdbResultPartial = imdbResult.slice(0, imdbResult.length > maxRequestedImdb ? maxRequestedImdb : imdbResult.length);

            return Promise.all(imdbResultPartial.map(async rawImdb => {
                try {
                    const omdbEntry = await (await fetch('https://0e53p7322m.execute-api.eu-central-1.amazonaws.com/release/movie/information/' + rawImdb.id)).json();
                    if (omdbEntry.Poster == 'N/A') {
                        Object.assign(omdbEntry, {Poster: missingPosterFb});
                    }
                    return Object.assign(omdbEntry, {valueField: JSON.stringify(omdbEntry)});
                } catch (err) {
                    SERVICE_CHANNEL.publish({
                        topic: srtPlayer.Descriptor.SERVICE.NOTIFICATION.SUB.NOTIFY,
                        data: {
                            msg: "Something goes wrong with the video search"
                        }
                    });
                    return {};
                }
            }));
        }
    });

//instant service does not correct initialize messageBus (in testfiles)
if (typeof exports === 'undefined' && typeof srtPlayer.MovieInformationService === 'function') {
    srtPlayer.MovieInformationService = srtPlayer.MovieInformationService();
}
