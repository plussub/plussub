...
// todo not a pinia store, maybe middleware to inject into each store
export const useStore = defineStore('contentScript', () => {
  if (!window.top) {
  ...
  const contentScriptLoadedObservable = inputObservable.pipe(
    ...
    tap(({ origin, source, data: { id } }) => connectionSubject.next({
      action: 'ADD',
      origin,
  ...
  const sendWithResponseObservable = combineLatest([sendWithResponseSubject, connectionObservable]).pipe(
    ...
    tap(([{ payload, mergeFn, responseSubject }, connections]) => {
      ...
      con.forEach(({ source }, idx) => source.postMessage({
        ...payload,
  ...
  const requestAllContentScriptsToRegister = () => {
    [...document.querySelectorAll('iframe'), { contentWindow: window }]
      ...
      .forEach(({ contentWindow, requestId }) => {
        ...
        contentWindow.postMessage({
          contentScriptInput: 'PING_REQUEST',
          requestId,
          extensionOrigin: EXTENSION_ORIGIN
        }, '*');
      });
  ...
  return {
    ...
    sendCommand(payload: Record<string, unknown>) {
      return sendAllSubject.next({ payload });
    },
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
