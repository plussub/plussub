...
import { distinct, filter, map, mergeMap, skip, takeUntil } from 'rxjs/operators';
...
export const init = ({ messageObservable, getElementFrom }: Payload) => {
  ...
  messageObservable
    ...
    .subscribe(({ el, messageEvent }) => (el.currentTime = messageEvent.data.time));
        ...
        fromEvent(el, 'timeupdate').pipe(
          takeUntil(
            messageObservable.pipe(
              filter<MessageEventFromPopup<string>, UnsubscribeToTimeUpdateMessageEvent>(
                (e): e is UnsubscribeToTimeUpdateMessageEvent => e.data.plusSubActionFromPopup === 'UNSUBSCRIBE_TO_TIME_UPDATE'
              ),
